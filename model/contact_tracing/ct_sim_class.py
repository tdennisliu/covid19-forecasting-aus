    ...
    def generate_times(self,  i=3.64, j=3.07, m=5.51, n=0.948, size=10000):
        ...
        while truncate:
            ...
            if np.all(np.array(p)<right_max):
                break
        self.test_times = self.t_t_offset + 0.75*np.random.gamma(self.t_t_shape, self.t_t_scale, size = size)
        self.notify_times = self.t_n_offset + 0.6*np.random.gamma(self.t_n_shape, self.t_n_scale, size = size)
        self.action_times = self.t_a_offset + 0.6*np.random.gamma(self.t_a_shape, self.t_a_scale, size = size)
        return None
