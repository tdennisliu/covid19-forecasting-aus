...
class Person:
    ...
    def __init__(self,parent, infection_time,detection_time, detected,category:str, test_time = 1000, notify_time = 1000, CT_time = 1000):
        """
...
class Forecast:
    ...
    def generate_times(self,  i=3.64, j=3.07, m=5.51, n=0.948, size=10000):
        ...
        self.CT_times = self.ct_offset + np.random.gamma(self.t_c_shape, self.t_c_scale, size = size)
    ...
    def iter_CT_time(self):
        """
        access Next CT_time
        """
        from itertools import cycle
        for time in cycle(self.CT_times):
            yield time 
        ...
        if curr_time ==0:
            ...
            self.get_CT_time = self.iter_CT_time()
                ...
                if ceil(inf_time) > self.cases.shape[0]:
                    ...
                    if new_case <= num_sympcases-1: #minus 1 as new_case ranges from 0 to num_offspring-1 
                        ...
                        if detection_rv < detect_prob:
                            ...
                            if self.people[parent_key].detected!=1:
                                ...
                                test_time = detect_time + next(self.get_test_time)
                                notify_time = test_time + next(self.get_notify_time)
                                CT_time = notify_time + next(self.get_CT_time)
                            else:
                                ...
                                #action_time = min(
                                 ...
                                 # Laura new times
                                 CT_time = min(
                                     self.people[parent_key].CT_time,
                                     detect_time + next(self.get_test_time) + next(self.get_notify_time) + next(self.get_CT_time)
                                 )
                        ...
                        if detection_rv < detect_prob:
                            ...
                            if self.people[parent_key].detected!=1:
                                ...
                                CT_time = detect_time + 2*(self.get_test_time + self.get_notify_time + self.get_CT_time)
                            else:
                                #parent was detected, and now case is routine detected
                                # Take minimum of routine isolation
                                # and parents isoaltion time
                    ...
                    if self.people[parent_key].detected==1:
                        ...
                        # Laura new times
                         ...
                         elif inf_time < self.people[parent_key].CT_time: 
                    ...
                    self.people[len(self.people)] = Person(parent_key, inf_time, detect_time,isdetected, 
                    category,test_time=test_time,notify_time=notify_time,CT_time=CT_time)
            #Laura
            if num_offspring>0:
                #Laura
    ...
    # Laura new times
    def simulate(self,end_time,sim,seeds,DAYS=2,pc=0.8,
    t_t_shape = 1/1, t_t_scale=1, t_n_shape = 1/1, t_n_scale = 1/1, t_c_shape = 1/1, t_c_scale=1/1):
        """
        ...
        self.t_c_shape = t_c_shape
        self.t_c_scale = t_c_scale
        self.t_c_offset = t_c_offset
